...
ht_t* ht_create(void) {
  ...
  hash_table->entries = malloc(sizeof(ht_entry_t*) * TABLE_SIZE);
...
void ht_set(ht_t* hash_table, const char* key, const char* value) {
  ...
  ht_entry_t* entry = hash_table->entries[slot];
...
void ht_del(ht_t* hash_table, const char* key) {
  ...
  ht_entry_t* entry = hash_table->entries[bucket];
  ...
  while (entry != NULL) {
    ...
    if (strcmp(entry->key, key) == 0) {
      ...
      if (entry->next == NULL && idx == 0) {
        hash_table->entries[bucket] = NULL;
      }

      // first item with a next entry
      if (entry->next != NULL && idx == 0) {
        hash_table->entries[bucket] = entry->next;
      }
